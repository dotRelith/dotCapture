...
namespace dotCapture
{
    public partial class SettingMenu : Form
    {
        ...
        {        
            ...
            //Setup color buttons
            backgroundColorButton.Click += (s, e) => { ChangeColor(ref backgroundColor); };
            textColorButton.Click += (s, e) => { ChangeColor(ref textColor); };
            hoverColorButton.Click += (s, e) => { ChangeColor(ref hoverColor); };
            //Setup dummy button
            dummyButton.Text = resourceManager.GetString("settings.dummy");
            dummyButton.Image = ResizeImage("dotCapture.icons.dummy.png", 24,24);
            //Subscribe event
            cancelButton.Click += (s, e) => { this.Hide(); };
            applyButton.Click += (s, e) => { this.Hide(); this.ApplyValues(); };
        ...
        private void ChangeColor(ref Color colorToChange)
        {
        ...
        private void UpdateDummyColors(object sender, EventArgs e)
        {
            ...
            Bitmap originalImage = (Bitmap)ResizeImage("dotCapture.icons.dummy.png", 24, 24);
